...
def main():
    ...
    if not btc_data and not eth_data:
        st.error(
            "❌ Unable to fetch price data. Please ensure the data pipeline is running."
        )
        st.code("bash scripts/start_pipeline.sh", language="bash")
        st.stop()
    ...
    if chart_style == "Candlestick":
        ...
        if symbol_view == "Both (Side by Side)":
            st.info(
                "Moving averages are only available in single-symbol view. Please select BTC or ETH."
            )
    ...
    if (btc_df is not None and not btc_df.empty) or (
        eth_df is not None and not eth_df.empty
    ):
    ...
    with col2:
        ...
        st.caption(
            "Data flows: CoinGecko → Kafka → Flink → Redis/PostgreSQL → FastAPI → Dashboard"
        )
