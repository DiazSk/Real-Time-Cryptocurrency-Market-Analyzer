...
class CryptoPriceProducer:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Initializing CryptoPriceProducer for {len(self.crypto_symbols)} cryptocurrencies"
        )
    ...
    def connect_kafka(self):
        ...
        try:
            ...
            logger.info(
                f"Connected to Kafka at {KAFKA_PRODUCER_CONFIG['bootstrap_servers']}"
            )
    ...
    def fetch_crypto_prices(self, symbols: List[str]) -> Optional[Dict]:
        ...
        for attempt in range(API_MAX_RETRIES):
            try:
                logger.debug(
                    f"Fetching prices (attempt {attempt + 1}/{API_MAX_RETRIES})"
                )
                ...
                if response.status_code == 200:
                    data = response.json()
                    logger.debug(
                        f"Successfully fetched prices for {len(data)} cryptocurrencies"
                    )
                    return data
    ...
    def create_price_message(self, symbol: str, data: Dict) -> Dict:
        ...
        message = {
            ...
            .isoformat()
            .replace("+00:00", "Z"),  # ISO 8601 with Z
    ...
    def produce_message(self, message: Dict):
        ...
        try:
            ...
            # Send to Kafka
            future = self.producer.send(KAFKA_TOPIC_CRYPTO_PRICES, key=key, value=value)
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                logger.info(
                    f"Total messages produced: {self.message_count}, Errors: {self.error_count}"
                )
                ...
                if PRODUCER_RUN_DURATION > 0:
                    ...
                    if elapsed >= PRODUCER_RUN_DURATION:
                        logger.info(
                            f"Reached run duration limit ({PRODUCER_RUN_DURATION}s), stopping..."
                        )
                        break
    ...
    def cleanup(self):
        ...
        if self.start_time:
            ...
            if self.message_count > 0:
                logger.info(
                    f"Average throughput: {self.message_count / total_runtime:.2f} messages/second"
                )
                success_rate = (
                    self.message_count / (self.message_count + self.error_count)
                ) * 100
